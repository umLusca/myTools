...
function shortName(string $fullName, bool $shortLastName = false): string
{
	$names = explode(" ", $fullName);
	$names = array_filter($names);
	if (count($names) >= 2) {
		return mb_convert_case(mb_strtolower(implode(' ', [$names[0], $shortLastName ? substr(end($names), 0, 1) . '.' : end($names)])), MB_CASE_TITLE);
...
{
	if ($string) {
		return trim($mb ? match ($mb) {
			default => $string,
			"lower", "l", "d" => mb_strtolower($string),
...
function postTreat(string $string = "", string|false $mb = false): false|string
{
...
function getTreat(string $string = "", string|false $mb = false): false|string
{
...
function realToNumber(string $string): float
{
	if (!empty($string)) {
